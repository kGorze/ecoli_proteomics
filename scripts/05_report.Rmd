...
- **UPS1**: 48 rekombinowanych białek ludzkich w mieszaninie **ekwimolarnej** (10,000 fmol każde)
...
| **Wariant 2** | VSN (Variance Stabilizing) | QRILC (Quantile Regression) |
...
```{r pca, eval=data_loaded, fig.cap="PCA po normalizacji VSN+QRILC. Wyraźna separacja grup UPS1 i UPS2."}
# PCA
...
fdr_combined <- bind_rows(
  ...
  benchmark$v2$fdr_summary %>% mutate(Wariant = "VSN+QRILC")
)
...
ggplot(fdr_combined, aes(x = factor(Threshold), y = Empirical_FDR_pct, fill = Wariant)) +
  ...
  scale_fill_manual(values = c("Median" = "#E41A1C", "VSN+QRILC" = "#377EB8")) +
  theme_minimal()
```
...
p2 <- plot_exp_obs(benchmark$v2$ups_comparison, "Wariant 2: VSN+QRILC",
...
better_fdr <- ifelse(benchmark$v1$empirical_fdr < benchmark$v2$empirical_fdr, 
                     "Wariant 1 (Median)", "Wariant 2 (VSN+QRILC)")
